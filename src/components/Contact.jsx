...
function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    // (Admin) - notify yourself first
    emailjs.send(SERVICE_ID, 'template_lmb5e0c', templateParams, { publicKey: PUBLIC_KEY })
      .then(() => {
        // Admin email succeeded, treat this as the primary success
        setStatus("success");
        setIsSending(false);
        e.target.reset();
        setTimeout(() => setStatus(""), 5000);

        // (Auto Reply) - fire separately, don't block success UX on it
        emailjs.send(SERVICE_ID, 'template_8go0n2t', templateParams, { publicKey: PUBLIC_KEY })
          .then((response) => {
            console.log('AUTO-REPLY SENT!', response.status, response.text);
          })
          .catch((err) => {
            // Log only, don't show error to user since admin email already went through
